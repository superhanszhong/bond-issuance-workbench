...
export default function Workbench() {
  ...
  async function loadWeek() {
    ...
    try {
      ...
      const [response, latestResponse, historyResponse] = await Promise.all([
        workbenchFetch(`/api/workbench?weekStart=${weekStart}`),
        workbenchFetch("/api/workbench?meta=latest"),
        workbenchFetch(`/api/workbench?startDate=${historyStart}&endDate=${historyEnd}`),
      ]);
  ...
  async function generateSpreadRange() {
    ...
    try {
      const response = await workbenchFetch(`/api/workbench?startDate=${analysisStart}&endDate=${analysisEnd}`);
  ...
  async function deleteImport(importId: string) {
    const response = await workbenchFetch(`/api/workbench?importId=${encodeURIComponent(importId)}`, { method: "DELETE" });
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="backup-actions">
          ...
          <button onClick={() => backupInput.current?.click()}><Upload size={15}/>导入数据备份</button>
          <input ref={backupInput} hidden type="file" accept=".json,application/json" onChange={event => {
            const file = event.target.files?.[0];
            if (file) void restoreLocalBackup(file).then(() => loadWeek()).then(() => setMessage("数据备份已导入")).catch(error => setMessage(error instanceof Error ? error.message : "导入失败"));
