...
import { fridayOf } from "./workbench";
...
const TEMPLATE_URL = `${process.env.NEXT_PUBLIC_BASE_PATH || ""}/templates/weekly-bond-report-template.docx`;
...
export async function buildWeeklyReportBlob({
  ...
  const source: ArrayBuffer = templateBytes ?? await fetch(TEMPLATE_URL).then((response) => {
